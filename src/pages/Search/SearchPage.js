...
const SearchPage = ({ products }) => {
  ...
  return (
    <div>
      ...
      Search results for <span className='font-semibold'>&ldquo;{state?.searchTerm}&rdquo;</span>
      </p>
      {prods.length > 0 ? (
        <ProductsGrid title='Searched Products' products={prods} />
      ) : <p className='p-8 text-center text-2xl font-semibold'>No Products matched your search term!</p>}
      <Footer />
    </div>
  );
};
